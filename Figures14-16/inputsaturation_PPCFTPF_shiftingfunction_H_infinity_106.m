...
%定义初值
x0=[106;0;1;1;0;0];   %分别对应状态1，状态2，辅助函数状态，自适应律
x=x0;
x_10=106-100;
xx=[];
u0=0.1223;
...
%     d(i)=0;
    ...
    if t(i)<Tf
        rho(i)=rho_0*exp(-lamda*t(i)/(Tf-t(i)))+rho_Tf+x(5,i);
    else
        rho(i)=rho_Tf+x(5,i);
    end
    ...
    %——————函数rho(i)的导数——————
    if t(i)<Tf
        rho_1(i)=-lamda*Tf*rho_0*exp(-lamda*t(i)/(Tf-t(i)))/((Tf-t(i))^2)+((-T11*x(5,i)+x(6,i)));
    else
        rho_1(i)=0+((-T11*x(5,i)+x(6,i)));
    end
